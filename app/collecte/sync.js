...
/** Thème clair/sombre de la PERSONNE derrière ce device (harmonisation multi-appareils).
    Renvoie {ok, theme:"dark"|"light"|""} ou null si l'endpoint n'existe pas encore. */
export async function fetchTheme(deviceId) {
  if (!deviceId) return null;
  const base = endpoint();
  const url = isGoogle()
    ? `${base}?action=theme&device_id=${encodeURIComponent(deviceId)}`
    : `${base || ""}/api/theme?device_id=${encodeURIComponent(deviceId)}`;
  try {
    const r = await fetch(url, { cache: "no-store" });
    if (!r.ok) return null;
    const data = await r.json();
...
/** Bascule le thème côté serveur (fire-and-forget, jamais bloquant : l'affichage local a déjà
    changé avant cet appel). */
export function setThemeRemote(deviceId, theme) {
...
export async function reconcile(onProgress = () => {}) {
  ...
  // Garde-fou auto-réparateur (2026-07-25) : un enregistrement local qui porte déjà un
  // server_id est PAR DÉFINITION confirmé, quel que soit son `status` — jamais à reposter.
  // Corrige tout de suite les enregistrements déjà mal marqués par un bug passé (ex.
