...
function renderProfileLangs() {
  ...
  // La langue créée rejoint « Mes langues d'appartenance » (aucun champ cible → arg vide).
...
async function saveContribution() {
  ...
  await DB.put(rec);
  _sessionClientIds.add(rec.client_id);   // « récent » sur Transcrire/Traduire pour cette session
...
function villageValue() {
  return $("#c-village-pere").value.trim();
}
function villageMereValue() {
  return $("#c-village-mere").value.trim();
}
...
function profileComplete() {
  return profileMissingFields().length === 0;
}
/** Liste PRÉCISE des champs obligatoires encore manquants (libellés lisibles, i18n), dans
    l'ordre du formulaire. Sert à la fois de source unique pour profileComplete() ET à afficher
    à l'utilisateur EXACTEMENT ce qui bloque (incident 2026-07-31 : un utilisateur qui avait
    « rempli toutes les cases » restait bloqué sans savoir lequel manquait — typiquement la
    langue paternelle/maternelle laissée sur « ➕ Ajouter ma langue » sans déclaration, qui
    AFFICHE un libellé mais vaut "" en réalité). */
function profileMissingFields() {
  const c = loadContributeur();
  const miss = [];
  if (!c.nom) miss.push(t("p.miss.nom"));
  if (!c.prenom) miss.push(t("p.miss.prenom"));
  if (!(c.village_pere || c.village)) miss.push(t("p.miss.village_pere"));
  if (!c.langue_pere) miss.push(t("p.miss.langue_pere"));
  if (!c.village_mere) miss.push(t("p.miss.village_mere"));
  if (!c.langue_mere) miss.push(t("p.miss.langue_mere"));
  if (!c.role) miss.push(t("p.miss.role"));
  if (!isEmailValid(c.email)) miss.push(t("p.miss.email"));   // marqué « * » → format valide exigé
  if (!c.indicatif) miss.push(t("p.miss.indicatif"));
  if (!c.telephone) miss.push(t("p.miss.telephone"));
  if (profileLangues().length === 0) miss.push(t("p.miss.langues"));
  if (!c.consentement) miss.push(t("p.miss.consent"));
  return miss;
}
...
// --- Tiroir de réglages (☰) : regroupe langue/à propos/aide/bugs/interface/thème, tout ce qui
// n'est pas quotidien (Brice 2026-07-25). Même patron que les autres dialogues (profile-gate,
// bugs…) : [hidden] + une classe d'état pour l'animation d'entrée/sortie.
let _drawerLinkJustClicked = false;   // cf. onHistoryNav : distingue un clic de lien d'un vrai retour
// Verrou de défilement de la page derrière (Brice 2026-07-25r : scroller DANS le tiroir ne
// doit avoir aucun effet sur la page en dessous). `overflow:hidden` seul sur body ne suffit
// pas de façon fiable sur mobile (rebond tactile qui « fuit »，iOS Safari) : on fige le body en
// `position:fixed` à son décalage actuel, la seule technique qui bloque vraiment le défilement
// tactile sur tous les navigateurs, puis on restaure exactement la position au scroll d'origine
// à la fermeture.
function _lockPageScroll() {
  // Idempotent : un double-tap sur ☰ (courant au doigt) appelait openSettingsDrawer() deux
  // fois avant la fin de l'animation d'ouverture → un 2e verrouillage lisait window.scrollY
  // à 0 (le body est déjà figé) et écrasait la position d'origine mémorisée, provoquant un
  // saut visible en haut de page à la fermeture (Brice 2026-07-25u, audit).
...
function openSettingsDrawer() {
  const d = $("#settings-drawer"); if (!d) return;
  d.hidden = false;
  requestAnimationFrame(() => { d.classList.add("is-open"); });
  const btn = $("#settings-toggle"); if (btn) btn.setAttribute("aria-expanded", "true");
  _lockPageScroll();
}
function closeSettingsDrawer() {
  const d = $("#settings-drawer"); if (!d || d.hidden) return;
  d.classList.remove("is-open");
  setTimeout(() => { d.hidden = true; }, 280);   // laisse l'animation de sortie se jouer
  const btn = $("#settings-toggle"); if (btn) btn.setAttribute("aria-expanded", "false");
  _unlockPageScroll();
}
...
function onHistoryNav(e) {
  ...
  // Même patron que le clavier ci-dessus, MAIS uniquement sur un vrai « Précédent » du
  // navigateur (bouton retour), jamais sur un simple clic de lien DANS le tiroir.
  // Piège (Brice 2026-07-25r, « aucun bouton ne pointe vers une page ») : un clic sur une
  // ancre `#foo` (navigation « même document ») déclenche AUSSI un `popstate` (en plus du
  // hashchange), donc `e.type === "popstate"` seul ne distingue PAS les deux cas — un 1er
  // essai sur ce seul critère annulait la navigation de TOUS les liens du tiroir. Un 2e essai
  // sur `e.state` (nul = navigation neuve, non nul = page déjà tracée) échouait aussi : la
  // toute 1re entrée d'historique (posée au boot) est délibérément `state: null`, donc un
  // retour vers CETTE entrée précise (ex. tiroir ouvert juste après le chargement) était pris
  // à tort pour un clic de lien. Distinction fiable : `_drawerLinkJustClicked`, posé
  // synchrone-ment par le gestionnaire de clic du tiroir juste avant que le navigateur ne
  // déclenche popstate/hashchange (cf. initSettingsDrawer).
  const drawer = $("#settings-drawer");
  if (e && e.type === "popstate" && !_drawerLinkJustClicked && drawer && !drawer.hidden) {
    closeSettingsDrawer();
    const route = viewToRoute(_currentView);
    if (route) { try { history.pushState({ v: _currentView }, "", "#/" + route); } catch (e) { /* ok */ } }
    return;
  ...
  if (res && Array.isArray(res.contributions) && res.contributions.length) {
    ...
    for (const s of res.contributions) {
      ...
      try {
        ...
        // ici, DB.pending()/DB.counts() (qui filtrent sur status!=="sent") le classaient à tort
        // comme « à renvoyer », et reconcile() le RE-POSTAIT à chaque cycle avec un client_id
        // synthétique que le serveur ne reconnaît jamais comme doublon → ligne dupliquée en base
        // (texte seul, sans l'audio d'origine) à CHAQUE tour de réconciliation.
        const sentAt = s.cree_le || s.recu_le || s.date || new Date().toISOString();
        await DB.put({
          ...
          created_at: sentAt, status: "sent", sent_at: sentAt, _rehydrated: true,
        });
      } catch (e) { /* quota/erreur : sans gravité */ }
    }
  }
...
const ORIGINS_DEFAULT_KEY = "langa-origines-defaut-v1";
/** SYNC UNIQUE (double lignage, 2026-07-25), même patron que applyCreditDefaultOnce() : la
    migration serveur (migrateOrigines()) ne touche que la base, jamais le localStorage de
    chaque appareil — sans ce filet, un profil déjà existant sur CET appareil deviendrait
    soudain « incomplet » (village_mere/langue_pere/langue_mere absents) dès la mise à jour.
    Présume village_mere=village_pere et langue_mere=langue_pere par défaut (comme côté
    serveur) ; jamais appliqué à un profil réellement neuf (le formulaire l'exige de toute
    façon). L'utilisateur reste libre de corriger ensuite dans son profil. */
function applyOriginsDefaultOnce() {
  try {
    if (localStorage.getItem(ORIGINS_DEFAULT_KEY)) return;
    const c = loadContributeur();
    const exists = !!(c && (c.nom || c.prenom || c.email));
    if (exists) {
      const villagePere = c.village_pere || c.village || "";
      const languePere = c.langue_pere || profileLangues()[0] || getCurrentLangId();
      let changed = false;
      if (villagePere && !c.village_pere) { c.village_pere = villagePere; changed = true; }
      if (villagePere && !c.village_mere) { c.village_mere = villagePere; changed = true; }
      if (languePere && !c.langue_pere) { c.langue_pere = languePere; changed = true; }
      if (languePere && !c.langue_mere) { c.langue_mere = languePere; changed = true; }
      if (changed) {
        saveContributeur(c);
        try { pushUserProfile(); } catch (e) { /* offline : repartira au prochain envoi */ }
      }
    }
    localStorage.setItem(ORIGINS_DEFAULT_KEY, "1");
  } catch (e) { /* jamais bloquant */ }
}
...
/** CTA proactif « Créer mon profil » (header, déplacé du hub le 2026-07-25s) + repère « 1·2·3 »
    de l'accueil (Brice 2026-07-25) : visibles seulement tant que le parcours de démarrage n'est
    pas terminé, pour qu'un nouvel arrivant sache immédiatement par où commencer. */
function updateHubOnboarding() {
  const done1 = profileComplete(), done2 = hasChosenLang();
  const cta = $("#btn-create-profile");
  if (cta) {
    cta.hidden = done1;
    if (!cta.dataset.bound) {
      cta.dataset.bound = "1";
      cta.addEventListener("click", () => { openProfile(false); showProfileGate(); });
    }
  }
  const ribbon = $("#onboard-ribbon");
  if (ribbon) {
    ribbon.hidden = done1 && done2;   // dès que profil + langue sont faits, plus besoin de repère
    const mark = (sel, done, current) => {
      const el = $(sel); if (!el) return;
      el.classList.toggle("is-done", done);
      el.classList.toggle("is-current", !done && current);
    };
    mark("#onboard-step-profile", done1, !done1);
    mark("#onboard-step-lang", done2, done1 && !done2);
    mark("#onboard-step-contrib", false, done1 && done2);
  }
}
...
// ASSIGNATION DIFFÉRÉE après création de langue (survit au reload de fin d'amorce) : quand la
// création est lancée depuis un sélecteur précis (langue paternelle/maternelle du profil, ou
// langue d'une demande), la langue nouvellement créée doit venir s'y placer une fois les 5 mots
// donnés. On persiste QUEL champ remplir, appliqué dans _amorceEnd (cf. plus bas).
const PENDING_LANG_ASSIGN_KEY = "langa-pending-langassign";
function _setPendingLangAssign(field) {
  try { if (field) localStorage.setItem(PENDING_LANG_ASSIGN_KEY, field); else localStorage.removeItem(PENDING_LANG_ASSIGN_KEY); }
  catch (e) { /* stockage indispo : la création reste valide, seule l'auto-sélection est perdue */ }
}
function _pendingLangAssign() { try { return localStorage.getItem(PENDING_LANG_ASSIGN_KEY) || ""; } catch (e) { return ""; } }
/** Ouvre le formulaire UNIQUE de création de langue (#lang-declare) EN PLACE dans le profil,
    suivi de l'amorce ≥5 mots (aucune langue n'est jamais créée sans ces 5 contributions — règle
    Brice). `assignField` (facultatif) = "langue_pere" | "langue_mere" : la langue créée s'y place
    automatiquement à la fin. Sans `assignField`, la langue rejoint « Mes langues d'appartenance »
    (comportement historique du bouton ➕ de cette section). */
function openDeclareInProfile(assignField) {
  _captureDeclareHome();
  _declareCtx = "profile";
  _setPendingLangAssign(assignField || "");
...
function _amorceAbort() {
  ...
  _setPendingLangAssign("");   // abandon : aucune langue créée → aucune assignation à faire
...
async function _amorceEnd() {
  ...
  // Assignation différée (2026-07-31) : si la création a été lancée depuis « Langue paternelle/
  // maternelle », la langue fraîchement créée vient s'y placer. Persisté sur le contributeur AVANT
  // le reload, donc fillProfileFields() la réaffiche automatiquement au rechargement du profil.
  const assign = _pendingLangAssign();
  if (assign === "langue_pere" || assign === "langue_mere") {
    const cc = loadContributeur(); cc[assign] = desc.id; saveContributeur(cc);
  }
  // Depuis la porte Demander : on mémorise la langue créée pour la présélectionner au retour
  // sur Demander (cf. enterDemander), et on y retourne directement après le reload.
  let reqReturn = false;
  if (assign === "req_langue") { try { localStorage.setItem("langa-pending-reqlang", desc.id); } catch (e) { /* ok */ } reqReturn = true; }
  _setPendingLangAssign("");
  ...
  // Explorer). Retour au profil si la déclaration en venait, à Demander si c'était son origine,
  // sinon l'accueil.
  setCurrentLangId(desc.id);
  const target = reqReturn ? "#/demander" : (_declareCtx === "profile" ? "#/profil" : "#/accueil");
  _declareCtx = null;
...
// Variété PAR UTILISATEUR (Brice 2026-07-25t) : mémorise les derniers mots proposés à CET
// appareil pour éviter de reproposer sans cesse les mêmes — deux appareils divergent donc
// naturellement dans le temps, même en piochant dans le même corpus partagé. Repli sur le
// pool complet si tout a déjà été montré récemment (jamais bloqué à sec).
const INCITE_HISTORY_KEY = "langa-incite-history";
const INCITE_HISTORY_MAX = 12;
function _incHistory() {
  try { const a = JSON.parse(localStorage.getItem(INCITE_HISTORY_KEY) || "[]"); return Array.isArray(a) ? a : []; } catch (e) { return []; }
}
function _incHistoryAdd(norm) {
  if (!norm) return;
  try {
    let a = _incHistory().filter((x) => x !== norm);
    a.push(norm);
    if (a.length > INCITE_HISTORY_MAX) a = a.slice(a.length - INCITE_HISTORY_MAX);
    localStorage.setItem(INCITE_HISTORY_KEY, JSON.stringify(a));
  } catch (e) { /* stockage indispo */ }
}
/** Écarte les items récemment montrés À CET appareil ; repli sur `pool` en entier si ça le vide. */
function _incFreshPool(pool, normOf) {
  const hist = new Set(_incHistory());
  const fresh = pool.filter((it) => !hist.has(normOf(it)));
...
const POPUP_ROTATE_MS = 180000;  // 3 min par popup avant de passer au suivant, tant qu'aucune action n'est prise (Brice 2026-07-25w : permanent, plus de disparition automatique)
...
// En PAUSE tant que l'utilisateur est sur une des 4 pages essentielles (Transcrire/Traduire/
// Explorer/Demander) : il est déjà en train de contribuer, l'interrompre par un popup
// d'incitation ressemblerait à du harcèlement (Brice 2026-07-25). Les popups en attente
// restent en file (rien n'est perdu) mais ne s'affichent ni ne tournent tant que c'est vrai.
let _pqPaused = false;
/** Les 4 espaces de travail essentiels : "app" couvre Transcrire ET Traduire (différenciées
    par `activity`), "explore" = Explorer, "demander" = Demander. */
function isEssentialWorkView(name) { return name === "app" || name === "explore" || name === "demander"; }
...
function _pqShow() {
  if (_pqPaused || !_pq.length) { _pqCurrent = null; _pqHideAllEls(); return; }
...
function _pqRestartTimer() {
  ...
  if (!_pqPaused && _pq.length > 1) _pqTimer = setInterval(_pqAdvance, POPUP_ROTATE_MS);
}
/** Met la file en pause (arrivée sur une page essentielle) ou la relance (on la quitte). */
function _pqSetPaused(paused) {
  if (paused === _pqPaused) return;
  _pqPaused = paused;
  if (paused) { clearInterval(_pqTimer); _pqTimer = 0; _pqHideAllEls(); }
  else if (_pqStarted && _pq.length) { _pqShow(); _pqRestartTimer(); }
}
...
function enqueuePopup(id, show) {
  ...
  _pq.push({ id, show });
  if (_pqStarted && !_pqPaused) { if (_pq.length === 1) _pqShow(); _pqRestartTimer(); }
}
...
function startPopupQueue() {
  ...
  setTimeout(() => { _pqStarted = true; if (!_pqPaused && _pq.length) { _pqShow(); _pqRestartTimer(); } }, POPUP_GRACE_MS);
}
...
async function pickRateCandidate() {
  try {
    ...
    // Variété par appareil : écarte ce qui a déjà été montré récemment À CET utilisateur
    // (cf. _incFreshPool) — sans ça, un pool de candidats réduit reproposait sans cesse les
    // mêmes 1-2 items, à tous les utilisateurs indistinctement (Brice 2026-07-25t).
    const pool = _incFreshPool(cands, (e) => normTxt((e.source_text || "").trim()));
...
async function pickIncitation() {
  ...
  // Variété par appareil (Brice 2026-07-25t) : écarte ce qui a déjà été montré récemment À CET
  // utilisateur AVANT même la recherche de référence sociale — sinon la boucle ci-dessous, qui
  // s'arrête au 1er match avec audio dans l'ordre renvoyé par la bibliothèque, biaisait vers
  // toujours les mêmes mots (ceux qui arrivent tôt dans cet ordre), pour tout le monde pareil.
  undone = _incFreshPool(undone, (it) => it.norm || normTxt(it.texte));
...
function renderIncitation(pick) {
  ...
  _incHistoryAdd(normTxt(w));   // mémorise ce qui a été RÉELLEMENT montré (variété par appareil)
...
function notifText(n, html) {
  ...
  // Type inconnu/futur ("announce"…) SANS texte fourni : jamais un message vide (Brice
  // 2026-07-25w, notification vide reçue) — repli générique plutôt qu'une bulle silencieuse.
  const raw = String(d.text || "").trim() || t("notif.generic");
...
async function maybeShowNotifPopup() {
  ...
  const fresh = _notifs.filter((n) => POP_TYPES[n.type]
    && (n.ts || 0) > seen && (n.ts || 0) > lastPop && !_isNotifRead(n)
    // Harmonisation multi-appareils (2026-07-25) : jamais reproposer en popup ce qui l'a déjà
    // été sur UN AUTRE appareil de la même personne, même si l'utilisateur ne l'a pas encore lu.
    && !n.shown);
  if (!fresh.length) return;
  const n = fresh[0];   // la plus récente (liste triée décroissante)
  const k = _notifKey(n);
  if (k) markNotifRead({ device_id: deviceId(), shown_id: k }).catch(() => {});   // best-effort
...
function _fillReqLangSelects() {
  ...
  if (sel) {
    ...
    // Aucune valeur par défaut : l'utilisateur DOIT choisir lui-même. « ➕ Ajouter ma langue »
    // en tête route vers le formulaire UNIQUE de création + amorce ≥5 mots (unifié 2026-07-31).
    sel.innerHTML =
...
// Même règle que slugLang (sigle ISO/≤3 lettres, unique).
function _slugLang(nom) { return slugLang(nom); }
/** « ➕ Ajouter ma langue » depuis la porte Demander : route vers le formulaire UNIQUE de
    création de langue + amorce ≥5 mots (unifié 2026-07-31 — plus de mini-form léger). La langue
    créée sera présélectionnée au retour sur Demander (assignation différée "req_langue"). */
function _onReqLangueChange() {
  const sel = $("#req-langue");
  if (!sel || sel.value !== "__new__") return;
  sel.value = "";
  refreshEnhancedSelects();
  _setPendingLangAssign("req_langue");
  openDeclareForm();
}
async function enterDemander(targetReqId) {
  ...
  _fillReqLangSelects();
  // Présélection après création d'une langue depuis cette porte (assignation différée) : la
  // langue tout juste créée devient le choix courant de la demande.
  try {
    const pend = localStorage.getItem("langa-pending-reqlang") || "";
    if (pend) {
      const sel = $("#req-langue");
      if (sel && Array.from(sel.options).some((o) => o.value === pend)) { sel.value = pend; refreshEnhancedSelects(); }
      localStorage.removeItem("langa-pending-reqlang");
    }
  } catch (e) { /* ok */ }
...
// Cache des sons Drive déjà téléchargés : fileId → URL de blob local, valable pour CETTE PAGE
// (le plus rapide, zéro I/O). Persisté AUSSI dans le Cache Storage du navigateur (2026-07-25,
// économie de données mobiles) : réécouter un son après un RECHARGEMENT ou une NOUVELLE VISITE
// ne le retélécharge plus jamais — un fileId Drive est immuable par construction (une correction
// crée un nouveau fichier, jamais une mutation), donc aucun hash/expiration n'est nécessaire ici,
// contrairement aux icônes statiques (cf. icons/manifest.json + MEDIA_CACHE dans sw.js, MÊME nom
// de cache que ci-dessous : les sons Drive y vivent sous une clé synthétique dédiée).
const _driveAudioBlob = new Map();
const MEDIA_CACHE_NAME = "collecte-media-v1";   // doit rester identique à MEDIA_CACHE (sw.js)
function driveAudioCacheKey(fileId) { return location.origin + "/__drive_audio__/" + encodeURIComponent(fileId); }
async function _driveAudioFromDisk(fileId) {
  if (!("caches" in window)) return null;
  try {
    const c = await caches.open(MEDIA_CACHE_NAME);
    const res = await c.match(driveAudioCacheKey(fileId));
    if (!res) return null;
    return URL.createObjectURL(await res.blob());
  } catch (e) { return null; }   // Cache Storage indisponible (contexte privé, quota…) : sans gravité
}
function _driveAudioToDisk(fileId, blob) {
  if (!("caches" in window)) return;
  caches.open(MEDIA_CACHE_NAME).then((c) => c.put(driveAudioCacheKey(fileId), new Response(blob))).catch(() => {});
}
...
async function loadDriveAudioInto(audio, fileId, setStatus) {
  const mem = _driveAudioBlob.get(fileId);
  if (mem) { audio.src = mem; audio.load(); if (setStatus) setStatus(""); return; }
  const fromDisk = await _driveAudioFromDisk(fileId);
  if (fromDisk) { _driveAudioBlob.set(fileId, fromDisk); audio.src = fromDisk; audio.load(); if (setStatus) setStatus(""); return; }
  ...
  try {
    ...
    if (data && data.ok && data.b64) {
      ...
      const blob = new Blob([arr], { type: data.mime || "audio/webm" });
      const url = URL.createObjectURL(blob);
      _driveAudioBlob.set(fileId, url);
      _driveAudioToDisk(fileId, blob);   // best-effort, jamais bloquant pour la lecture en cours
...
function updateProfileGate() {
  ...
  if (lock) {
    lock.hidden = ok;
    // Message PRÉCIS : liste les champs qui manquent réellement, plutôt qu'un « complète tous
    // les champs » générique qui laissait l'utilisateur deviner (incident 2026-07-31).
    if (!ok) lock.textContent = "🔒 " + ti("profile.lock.missing", { fields: missing.join(", ") });
  }
}
...
/** `sync` = true → répercute le changement côté serveur, PAR PERSONNE (harmonisation multi-
    appareils, 2026-07-25) : fire-and-forget, jamais bloquant, l'affichage local a déjà changé.
    false pour l'application initiale au boot ou l'adoption d'une valeur déjà reçue du serveur
    (jamais renvoyer au serveur ce qu'il vient de nous dire → pas de va-et-vient inutile). */
function applyTheme(mode, sync) {
  ...
  if (sync) { try { setThemeRemote(deviceId(), mode).catch(() => {}); } catch (e) { /* best-effort */ } }
}
function initTheme() {
  const cur = document.documentElement.getAttribute("data-theme") === "dark" ? "dark" : "light";
  applyTheme(cur, false);   // valeur locale déjà posée par le script <head> : pas de flash
  const btn = $("#theme-toggle");
  if (btn) btn.addEventListener("click", () => {
    const now = document.documentElement.getAttribute("data-theme") === "dark" ? "light" : "dark";
    applyTheme(now, true);
  });
  // Harmonisation : si la PERSONNE a déjà choisi un thème sur un autre appareil, on l'adopte ICI
  // aussi (best-effort, jamais bloquant — hors ligne ou ancien backend -> on garde le thème local).
  fetchTheme(deviceId()).then((res) => {
    if (res && res.theme && res.theme !== cur) applyTheme(res.theme, false);
  }).catch(() => {});
}
...
async function refresh() {
  ...
  // le DB local sans filtre de session. « À renvoyer » reste volontairement NON filtré : un envoi
  ...
  if (grpS) grpS.hidden = sentRecent.length === 0;
  // Lien vers l'historique complet, seulement s'il existe des envois HORS de la session en cours
  // (sinon rien à ajouter : tout ce qui est envoyé est déjà listé ici).
  const moreLink = $("#send-more-link");
  if (moreLink) moreLink.hidden = sent.length <= sentRecent.length;
  ...
  fill("#list-sent", sentRecent, true);
...
// Contributions faites PAR CET APPAREIL durant CETTE SESSION (Brice 2026-07-25 : au sens strict
// du mot, tant que le navigateur reste ouvert sur ce device sans recharger le site). Jamais
// persisté (une Set en mémoire se réinitialise naturellement au rechargement, exactement la
// définition voulue) : sert à limiter la liste « Envoyés » de Transcrire/Traduire à ce qui vient
// d'être fait ici et maintenant. L'historique COMPLET reste dans le Profil (#my-contribs), qui
// lit tout DB local (y compris les contributions rapatriées d'autres appareils/sessions).
const _sessionClientIds = new Set();
...
function openThanksModal(total, justConfirmed) {
  try {
    ...
    ov.querySelector(".thanks-card").dataset.skin = c.skin || "or";
...
function mountShareBars() {
  ...
  ["share-about", "share-foot", "share-present"].forEach((id) => {
...
/** Ligne « repérée le {date} à {heure} » de la bannière — pas de détail de contenu (Brice
    2026-07-25x : juste signaler qu'une mise à jour existe, sans lister les changements). */
function renderUpdateWhen() {
  const el = $("#update-when"); if (!el) return;
  const now = new Date();
  const locale = getUiLang() === "en" ? "en-GB" : "fr-FR";
  const when = now.toLocaleString(locale, { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" });
  el.textContent = ti("update.detected", { when });
}
...
async function applyUpdate() {
  ...
  try {
    // 1) purge d'abord la coquille (code) — JAMAIS le cache des médias (icons/*, séparé,
    //    "collecte-media-", cf. sw.js) : un bug ici purgeait TOUT sans distinction, donc
    //    CHAQUE clic « Mettre à jour » forçait un retéléchargement complet des images/audios
    //    déjà en cache (des dizaines de Mo) sur le réseau mobile de l'utilisateur — la cause
    //    probable du ralentissement remonté par Brice (2026-07-25u), alors que seul le code
    //    (quelques centaines de Ko) a besoin d'être renouvelé à chaque version.
    if (window.caches) {
      const keys = await caches.keys();
      await Promise.all(keys.filter((k) => !k.startsWith("collecte-media-")).map((k) => caches.delete(k)));
    }
...
/** Généralisé (2026-07-25, double lignage) : `suffix` cible le combo village PATERNEL
    ("-pere") ou MATERNEL ("-mere") — même logique, deux champs indépendants. */
function setupVillageCombo(suffix) {
  const combo = $("#village-combo" + suffix);
  const input = $("#c-village" + suffix);
  const toggle = $("#village-toggle" + suffix);
  const list = $("#village-list" + suffix);
...
// --- « Ajouter ma langue » depuis « Langue paternelle/maternelle » -------------------------
// UNIFIÉ (Brice 2026-07-31) : plus AUCUN mini-formulaire dédié. On route vers le formulaire
// UNIQUE de création de langue (#lang-declare) + l'amorce ≥5 mots, exactement comme partout
// ailleurs — aucune langue ne peut être créée sans ses 5 contributions. La langue créée vient
// se placer dans le champ (langue_pere/mere) qui a lancé la création (assignation différée).
function _onOriginLangueChange(which) {
  const sel = $(which === "mere" ? "#c-langue-mere" : "#c-langue-pere");
  if (!sel || sel.value !== "__new__") return;
  // On remet le sélecteur sur le placeholder : "__new__" n'est pas une vraie valeur, et la
  // langue réelle sera posée ici automatiquement à la fin de l'amorce (cf. _amorceEnd).
  sel.value = "";
  refreshEnhancedSelects();
  openDeclareInProfile(which === "mere" ? "langue_mere" : "langue_pere");
}
...
const FIELD_TIPS = {
  ...
  "c-village-pere": { fr: "Le village ou quartier de ton père. Les mots changent d'un village à l'autre : cette information situe ta variante paternelle.",
                       en: "Your father's village or neighbourhood. Words change from one village to another: this places your paternal variety." },
  "c-village-mere": { fr: "Le village ou quartier de ta mère. En Afrique, on a souvent deux origines distinctes : celle du père et celle de la mère.",
                       en: "Your mother's village or neighbourhood. In Africa, it's common to have two distinct origins: your father's and your mother's." },
  "c-langue-pere": { fr: "La langue du côté de ton père. Si tes deux parents parlent la même langue, choisis-la deux fois : c'est normal.",
                      en: "The language from your father's side. If both your parents speak the same language, pick it twice: that's normal." },
  "c-langue-mere": { fr: "La langue du côté de ta mère. Ces deux informations aident aussi à te distinguer d'un homonyme si tu changes d'appareil.",
                      en: "The language from your mother's side. These two pieces of information also help tell you apart from a namesake if you switch devices." },
...
function initContributeur() {
  setupVillageCombo("-pere");
  setupVillageCombo("-mere");
  initIndicatifs();                       // remplit d'abord les options d'indicatif
  initLangueOrigineSelects();             // idem pour langue paternelle/maternelle
  ...
  ["#c-langue-pere", "#c-langue-mere"].forEach((sel) => {
    $(sel).addEventListener("change", () => {
      const v = $(sel).value;
      // "__new__" (➕ Ajouter ma langue) n'est PAS une vraie langue : traité par
      // _onOriginLangueChange ci-dessous (ouvre le formulaire UNIQUE de création + amorce).
      if (v && v !== "__new__") { addProfileLangue(v); renderProfileLangs(); }
    });
  });
  // « ➕ Ajouter ma langue » dans les sélecteurs père/maternel : route vers le formulaire UNIQUE
  // de création de langue (#lang-declare) + amorce ≥5 mots (Brice 2026-07-31, plus de mini-form dédié).
...
function initEvents() {
  ...
  $("#btn-profile-continue").addEventListener("click", () => {
    if (!profileComplete()) return;
    // Double lignage (2026-07-25) : à la 1re complétion d'un profil neuf (aucune langue
    // d'appartenance encore choisie), pré-remplit « Mes langues » avec langue_pere/langue_mere
    // — un point de départ pratique, sans lien permanent : l'utilisateur reste libre d'ajouter/
    // retirer des langues d'appartenance ensuite, indépendamment de ses origines.
    if (!profileLangues().length) {
      const c = loadContributeur();
      if (c.langue_pere) addProfileLangue(c.langue_pere);
      if (c.langue_mere && c.langue_mere !== c.langue_pere) addProfileLangue(c.langue_mere);
    }
    ...
    const oldLang = getCurrentLangId();
    const primary = profileLangues()[0] || oldLang;
    // Enregistre EXPLICITEMENT le choix, même si `primary` coïncide avec le repli par défaut
    // ("nge") : sinon hasChosenLang() (qui teste la présence de la clé, pas sa valeur) resterait
    // faux pour toujours — cas réel via le CTA « Créer mon profil », qui saute l'écran de choix
    // de langue (#/langue) et amène directement ici (bug trouvé en vérifiant le repère 1·2·3).
    if (primary) setCurrentLangId(primary);
    if (primary && primary !== oldLang) {
  ...
  // Tiroir de réglages (☰, Brice 2026-07-25) : regroupe langue/à propos/aide/bugs/interface/thème.
  initSettingsDrawer();
...
async function main() {
  ...
  applyOriginsDefaultOnce();  // double lignage : village_mere/langue_pere/langue_mere présumés UNE fois aux profils existants
  initKeyboard();
  initKeyboardReveal();
  initPropCategories();
